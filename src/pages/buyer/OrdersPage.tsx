...
function useSellerLocations(sellerIds: string[]) {
  return useQuery({
    queryKey: ['seller-locations', sellerIds],
    queryFn: async () => {
...
export default function OrdersPage() {
  ...
  // Collect unique seller IDs across all orders
  const sellerIds = [...new Set(orders.flatMap(o => o.items.map(i => i.seller_id)))];
  const { data: sellerLocations = {} } = useSellerLocations(sellerIds);

  const getOrderDistance = (order: typeof orders[0]) => {
    if (!buyerLocation?.latitude || !buyerLocation?.longitude) return null;
    // Use first item's seller as primary seller for distance
          ...
          {orders.map(order => {
            ...
            return (
              <div key={order.id} className="bg-card border rounded-xl p-5">
                ...
                {(distance != null || estimatedTime) && order.status !== 'delivered' && order.status !== 'cancelled' && (
                  <div className="flex flex-wrap gap-3 mt-2 mb-2">
                    {distance != null && (
                      <span className="inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full bg-accent text-accent-foreground">
                        <MapPin className="w-3 h-3" /> {distance.toFixed(1)} km away
                      </span>
                    )}
                    {estimatedTime && (
                      <span className="inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full bg-secondary/20 text-secondary-foreground">
                        <Clock className="w-3 h-3" /> Est. Delivery: {estimatedTime}
                      </span>
                    )}
                  </div>
                )}
