...
export default function CartPage() {
  const { cart, removeFromCart, updateCartQuantity, cartTotal, placeOrder, clearCart } = useStore();
  const { user } = useAuth();
            ...
            {cart.map(item => (
              <div key={item.id} className="flex gap-4 bg-card border rounded-xl p-4">
                ...
                <div className="flex-1 min-w-0">
                  <Link to={`/product/${item.product.id}`} className="font-medium text-sm hover:text-primary">{item.product.name}</Link>
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  <div className="flex items-center border rounded-lg">
                    <button onClick={() => updateCartQuantity(item.product.id, item.quantity - 1)} className="px-2 py-1 hover:bg-muted"><Minus className="w-3 h-3" /></button>
                    <span className="px-3 py-1 border-x text-sm">{item.quantity}</span>
                    <button onClick={() => updateCartQuantity(item.product.id, item.quantity + 1)} className="px-2 py-1 hover:bg-muted"><Plus className="w-3 h-3" /></button>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="bg-card border rounded-xl p-6 h-fit sticky top-32">
            ...
            {showCheckout ? (
              <div className="mt-4 space-y-3">
                ...
                <Button className="w-full" onClick={handleOrder}><ShoppingBag className="w-4 h-4 mr-2" />Place Order</Button>
