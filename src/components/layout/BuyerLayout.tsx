      ...
      <header className="sticky top-0 z-50 border-b bg-card shadow-card">
        ...
        {mobileMenu && (
          <div className="sm:hidden border-t p-4 bg-card space-y-2">
            ...
            <Link to="/profile" className="block p-2 rounded hover:bg-muted" onClick={() => setMobileMenu(false)}>Profile</Link>
            <button onClick={() => { logout(); navigate('/login'); }} className="block w-full text-left p-2 rounded hover:bg-muted text-destructive">Logout</button>
          </div>
        )}
      </header>
